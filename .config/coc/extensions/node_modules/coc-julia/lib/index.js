...
const ctx_1 = __webpack_require__(2);
async function activate(context) {
    ...
    if (!bin) {
        coc_nvim_1.workspace.showMessage(`Can't find julia`, 'warning');
        return;
    }
    const missing = await ctx.resolveMissingPkgs();
    if (missing.length) {
        coc_nvim_1.workspace.showMessage(`Julia module missing: ${missing.toString()}. You need to install them first.`, 'warning');
        return;
    }
...
const os_1 = __importDefault(__webpack_require__(5));
const vscode_languageserver_protocol_1 = __webpack_require__(6);
const which_1 = __importDefault(__webpack_require__(35));
const JuliaFullTextNotification = new vscode_languageserver_protocol_1.NotificationType('julia/getFullText');
class Config {
    constructor() {
        this.cfg = coc_nvim_1.workspace.getConfiguration('julia');
    }
    get enabled() {
...
class Ctx {
    ...
    resolveJuliaBin() {
        ...
        if (bin.startsWith('~')) {
            bin = os_1.default.homedir() + bin.slice(1);
        }
        if (bin && fs_1.default.existsSync(bin)) {
            return bin;
        }
        const cmd = process.platform === 'win32' ? 'julia.exe' : 'julia';
        return which_1.default.sync(cmd, { nothrow: true });
    }
    async resolveMissingPkgs() {
        const bin = this.resolveJuliaBin();
        const installed = child_process_1.execSync(`${bin} -e "using Pkg; Pkg.status()"`)
            .toString()
            .split('\n');
        const missing = [];
    ...
    async resolveEnvPath() {
        ...
        const bin = this.resolveJuliaBin();
        return child_process_1.execSync(`${bin} --startup-file=no --history-file=no -e "using Pkg; println(dirname(Pkg.Types.Context().env.project_file))"`)
            .toString()
            .trim();
    }
    async startServer() {
        ...
        const outputChannel = coc_nvim_1.workspace.createOutputChannel('Julia Language Server Trace');
        const serverOptions = { command: bin, args };
        const clientOptions = {
            documentSelector: ['julia', 'juliamarkdown'],
            initializationOptions: coc_nvim_1.workspace.getConfiguration('julia'),
            synchronize: { configurationSection: ['julia.lint', 'julia.format'], fileEvents: coc_nvim_1.workspace.createFileSystemWatcher('**/*.{jl,jmd}') },
            progressOnInitialization: true,
            outputChannel,
        };
        const client = new coc_nvim_1.LanguageClient('julia', 'Julia Language Server', serverOptions, clientOptions);
...
const isexe = __webpack_require__(36)
